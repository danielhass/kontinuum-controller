...
kubectl create ns kontinuum-controller --dry-run=client -o yaml | kubectl apply -f -

echo -n "> Please enter the Helm repository URL: "
read HELM_URL

echo "> Creating Flux Helm source on target cluster: "
flux create -n kontinuum-controller source helm kontinuum-catalog --interval=1h --url=${HELM_URL}
...
flux create source bucket ${S3_BUCKET} --bucket-name=${S3_BUCKET} --endpoint=${S3_URL} --provider=generic --interval=1m --access-key=${S3_ACCESS_KEY} --secret-key=${S3_SECRET_KEY}

echo -n "> Please enter the new target cluster name (used to reference it on the control-plane): "
read TARGET_NAME

echo "> Create Flux S3 bucket reference on target cluster: "
flux create kustomization ${S3_BUCKET} --source=Bucket/${S3_BUCKET} --path="./${TARGET_NAME}" --interval=1m --prune
